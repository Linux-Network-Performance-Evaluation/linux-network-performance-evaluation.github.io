...
function DevelopmentQA() {
    ...
    const ipv4_datasets = {
        'Delay': {
            headers: ['Packet Size', 'Ubuntu IPv4 TCP', 'Ubuntu IPv4 UDP', 'Fedora IPv4 TCP', 'Fedora IPv4 UDP', 'Kali IPv4 TCP', 'Kali IPv4 UDP'],
            rows: [
    ...
    const ipv6_datasets = {
        'Delay': {
            headers: ['Packet Size', 'Ubuntu IPv6 TCP', 'Ubuntu IPv6 UDP', 'Fedora IPv6 TCP', 'Fedora IPv6 UDP', 'Kali IPv6 TCP', 'Kali IPv6 UDP'],
            rows: [
    ...
    const formatValue = (value, unit) => {
        if (unit === 'bps') {
            return value.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) + ' bps';
        } else if (unit === '%') {
                    ...
                    {Object.entries(datasets).map(([metric, data]) => (
                        <Tab eventKey={metric} title={metric} key={metric}>
                            <div className="mb-2 text-center">
                                <small className="text-muted">All values include units: {data.unit === 'bps' ? 'bits per second' : data.unit === '%' ? 'percentage' : 'milliseconds'}</small>
                            </div>
                                    ...
                                    <thead className="table-dark">
                                        <tr>
                                            {data.headers.map((header, index) => (
                                                <th key={index} className={`text-center ${header.includes('UDP') ? 'udp-header' : ''}`}>
                                                    {index === 0 ? header : (
                                                        <div>
                                        ...
                                        {data.rows.map((row, rowIndex) => (
                                            <tr key={rowIndex}>
                                                {row.map((cell, cellIndex) => {
                                                    const isUdpColumn = cellIndex > 0 && data.headers[cellIndex].includes('UDP');
                                                    return (
                                                        <td key={cellIndex} className={`text-center ${isUdpColumn ? 'udp-cell' : ''}`}>
                                                            {cellIndex === 0 ? cell : formatValue(cell, data.unit)}
                                                        </td>
                                                    );
                                                })}
                                            </tr>
        ...
        <main>
            <Container>
                ...
                <section className="content-section">
                    <Row>
                        <Col>
                            <h2 className="text-center mb-4">Network Performance Evaluation Results</h2>
                            <p className="text-center mb-4">
                                Comprehensive performance data collected from our Linux network evaluation tests. 
                                Data includes delay, jitter, throughput, and packet loss measurements across different 
                                packet sizes for Ubuntu, Fedora, and Kali Linux distributions using both TCP and UDP protocols.
                            </p>
                            ...
                            <Row className="mt-4">
                                <Col md={6} className="mb-3">
                                    <Card className="accent-bg h-100">
                                        <Card.Body>
                                            <Card.Title className="h5">Data Collection Methodology</Card.Title>
                                            <ul className="mb-0">
                                ...
                                <Col md={6} className="mb-3">
                                    <Card className="accent-bg h-100">
                                        <Card.Body>
                                            <Card.Title className="h5">Key Metrics Measured</Card.Title>
                                            <ul className="mb-0">
                                                <li><strong>Delay:</strong> End-to-end packet latency (ms)</li>
                                                <li><strong>Jitter:</strong> Variation in packet delay (ms)</li>
                                                <li><strong>Throughput:</strong> Data transfer rate (bps)</li>
